...
# the same process can be followed for any window size
grid_mcc_rfc_liko = []
grid_params_rfc_liko =[]
...
for v in  allmixdfs21:
    ...
    X_enc = X[Xcols].apply(lambda k: pd.Series(liko_encode(k)),axis=1) # encoding the X using the LiKo matrix and function above
    ...
    grid_mcc_rfc_liko.append(metrics.matthews_corrcoef(y_test, rfc_predictions).round(3))
    grid_params_rfc_liko.append(grid.best_params_)
